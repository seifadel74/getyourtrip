...
const Destinations = () => {
  ...
  const [selectedCountry, setSelectedCountry] = useState(t('destinations.all'));

  // Get unique countries for filter
  const countries = [t('destinations.all'), ...Array.from(new Set(destinations.map(dest => dest.country)))];
  ...
  const filteredDestinations = destinations.filter((destination: CustomDestination) => {
    ...
    const matchesCountry = selectedCountry === t('destinations.all') || destination.country === selectedCountry;
      ...
      <div className={styles.searchSection}>
        <div className={styles.searchContainer}>
          ...
          <div className={styles.filterGroup}>
            <label htmlFor="country-filter" className={styles.filterLabel}>{t('destinations.filterByCountry')}</label>
            <select
              id="country-filter"
        ...
        <div className={styles.destinationsGrid}>
          {filteredDestinations.length > 0 ? (
              ...
              >
                ...
                <div className={styles.destinationInfo}>
                  ...
                  <div className={styles.rating}>
                    ...
                    <span className={styles.reviews}>({destination.reviews} {t('destinations.reviews')})</span>
                  </div>
                </div>
              </motion.div>
            ))
            ...
            <div className={styles.noResults}>
              <h3>{t('destinations.noResults')}</h3>
              <p>{t('destinations.tryAdjusting')}</p>
            </div>
          )}
        </div>
      </div>
