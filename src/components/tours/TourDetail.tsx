...
const TourDetail = () => {
  ...
  ), [tour?.reviews, setIsReviewFormOpen]);
  ...
  useEffect(() => {
    const fetchTour = async () => {
      try {
        ...
        let reviews = [];
        try {
          const reviewsResponse = await api.reviews.getByTour(tourId);
        ...
        const calculateAverageRating = (reviewsList: any[]): number => {
          ...
          return Math.round((sum / reviewsList.length) * 100) / 100;
        };
        ...
        const tour: Tour = {
          ...
          image: imageValue,
          price: apiTour.price,
          rating: reviews.length > 0 ? calculateAverageRating(reviews) : (apiTour.rating || 0),
          description: apiTour.description || '',
          duration: apiTour.duration || '',
          included: parseArrayField(apiTour.included),
          excluded: parseArrayField(apiTour.excluded),
          highlights: parseArrayField(apiTour.highlights),
          reviews: reviews,
          ...
          itinerary: parseArrayField(apiTour.itinerary),
          countries: parseArrayField(apiTour.countries),
          languages: parseArrayField(apiTour.languages)
        };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.mainContainer}>
              ...
              <div className={styles.ratingContainer}>
                <div className={styles.rating}>
                  <span className={styles.ratingValue}>
                    ...
                    <span className={styles.ratingReviews}>({tour.reviews?.length || 0} Reviews)</span>
                  </span>
                </div>
              </div>
            </div>
        ...
        {/* Review Form Modal */}
        {isReviewFormOpen && tour && (
          <ReviewForm
            tourId={tour.id}
            onReviewSubmitted={async () => {
              // Refresh reviews after submission
              try {
                const reviewsResponse = await api.reviews.getByTour(tour.id);
                if (reviewsResponse.success && reviewsResponse.data) {
                  const updatedReviews = reviewsResponse.data.data || [];
                  ...
                  setTour(prev => prev ? { ...prev, reviews: updatedReviews, rating: newRating } : null);
                }
              } catch (err) {
