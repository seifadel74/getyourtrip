...
export const api = {
  ...
  contact: {
    submit: async (form: ContactForm): Promise<ApiResponse<void>> => {
      return apiRequest<void>('/contact-submissions', {
        method: 'POST',
    ...
    getAll: async (params: {
      ...
      const queryString = queryParams.toString();
      const url = `/contact-submissions${queryString ? `?${queryString}` : ''}`;
      return apiRequest<any[]>(url);
    },
    getById: async (id: number): Promise<ApiResponse<any>> => {
      return apiRequest<any>(`/contact-submissions/${id}`);
    },
    markAsRead: async (id: number): Promise<ApiResponse<void>> => {
      return apiRequest<void>(`/contact-submissions/${id}/mark-as-read`, {
        method: 'POST',
      });
    },
  },
  ...
  // Reviews
  reviews: {
    getByTour: async (tourId: number, params?: {
      per_page?: number;
      page?: number;
    }): Promise<ApiResponse<any>> => {
      const queryParams = new URLSearchParams();
      queryParams.append('tour_id', String(tourId));
      if (params) {
        Object.entries(params).forEach(([key, value]) => {
          if (value !== undefined && value !== null) {
            queryParams.append(key, String(value));
      ...
      const queryString = queryParams.toString();
      return apiRequest<any>(`/reviews${queryString ? `?${queryString}` : ''}`);
    },
    ...
    update: async (id: number, review: Partial<{
      is_approved: boolean;
      comment: string;
    }>): Promise<ApiResponse<any>> => {
      return apiRequest<any>(`/reviews/${id}`, {
    ...
    getTourRatingSummary: async (tourId: number): Promise<ApiResponse<any>> => {
      return apiRequest<any>(`/tours/${tourId}/rating-summary`);
    },
  },
};
